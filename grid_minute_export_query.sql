/*
 * Parameterized one-minute dataset query used by backfill_minute_bars.py.
 * Python supplies start_time, end_time (exclusive), and a text[] of tickers.
 */
WITH settings AS (
    SELECT
...
selected_tickers AS (
    SELECT UNNEST(%(tickers)s::text[]) AS tickersymbol
),
